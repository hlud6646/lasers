...
# -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- - -- -- -- - -- #
def normal(q, e):
    """ Return unit inward pointing normal vetor to the ellipse
        with eccentricity e, at the point q. """
...
def reflected_unit_vector(d, q, e):
    """ d is vector representing a ray which meets the ellipse
        (with eccentricity e) at q.  Returns a vector representing
        the reflected ray leaving q. """
    n = normal(q, e)
    d = 2*d.dot(n)*n - d
    return d / norm(d)
...
def next_intersection(p, r, e):
    """ Given a ray leaving p in the direction of r, return the point
        where this ray next meets the ellipse. """
    c = 1-e*e
...
def init_transforms(height, width):
    """ Return vectorized transformations from
        (-1, 1)x(-1, 1) to (0, width)x(0, height).
        These are a convenience for making drawing on the image frame cleaner.
        It's a compromise that every plotting method must call this method
        to create its own transformations, but nesseccary in order that
        height and width can stay out of global namespace."""
    @np.vectorize
    def w(x):
